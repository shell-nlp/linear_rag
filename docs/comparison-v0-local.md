...
同次运行中，当前版本临时索引约 9.80 秒，`v0` 约 12.29 秒。索引时间包含真实
Embedding、ES 写入以及 `v0` 的 Neo4j 节点/关系写入，但不包含 PDF 解析和对象存储。

主要差异：

- 全图 `linear` 与 `linear_local` 在全部 Recall@K 上完全相同；在这份 90 段小图上，
  全图路径中位耗时还略低。该结果只说明当前数据规模下候选裁剪没有带来收益，不能
  外推到百万级图。
- 图路径在 Recall@1/3/5 分别比 Hybrid-RRF 多 19、8、1 条命中，说明图排序对
  前几名更有效。
...
- 全图 `linear` 与 `linear_local` 的召回指标完全相同，`All@10` 和 `All@15`
  分别为 85% 和 90%；在这份 90 段小图上，全图路径的查询中位耗时还略低。
  该现象不能外推到大规模图，百万级数据仍必须使用候选裁剪和有界子图。
