...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| ES 纯向量 | 51/152 | 88/152 | 104/152 | 116/152 | 121/152 | 130/152 | 0.057 秒 |
| ES 向量 + BM25 + RRF | 88/152 | 110/152 | 120/152 | 130/152 | 132/152 | 141/152 | 0.154 秒 |
| 全图 `linear` | 107/152 | 118/152 | 121/152 | 129/152 | 131/152 | 135/152 | 1.724 秒 |
| 当前 `linear_local` | 107/152 | 118/152 | 121/152 | 129/152 | 131/152 | 135/152 | 1.910 秒 |
| 实际 `v0` | 75/152 | 105/152 | 110/152 | 115/152 | 119/152 | 119/152 | 2.591 秒 |

这里的纯向量和 Hybrid-RRF 都是主召回基线，不追加实体扩展和相邻段落融合；
Hybrid-RRF 指 ES 分别执行向量与 BM25 后，再按排名执行 RRF。同次运行中，当前版本
临时索引约 9.80 秒，`v0` 约 12.29 秒。152 条问题来自 76 个唯一实体，每个实体
使用两种问法。`linear_local` 在 Recall@1/3/5 最高，但 Hybrid-RRF 从 Recall@8
开始反超，并在 Recall@15 达到 141/152；说明局部图排序更适合前列精排，BM25
补充更有利于扩大候选召回。全图 `linear` 与 `linear_local` 在该小图上召回完全相同，
全图路径耗时略低；该结果不能外推到大规模图。该实验仍是内部 PDF 上的弱标注评测，
不能替代人工相关性标注、公开中文基准或生产规模测试。完整环境、结论和复跑命令见
...
- **单跳精排优先用图路径**：`linear`/`linear_local` 在 Recall@1/3/5 均领先，
  说明实体—句子桥接和 PPR 更适合把正确答案推到前几名。
- **深层候选优先用 Hybrid-RRF**：Hybrid-RRF 从 Recall@8 开始反超，Recall@15
  达到 141/152；BM25 与向量的排名融合比纯向量覆盖更广。
- **多跳必须看 All@K**：Hybrid-RRF 的 `Any@15` 虽为 100%，但 `All@15` 只有 75%；
  图路径的 `All@15` 为 90%，说明它能更稳定地补齐第二跳。
- **推荐生产形态是两阶段**：先用向量/BM25/Hybrid-RRF 快速扩大候选，再用
  `linear_local` 做有界子图精排和多跳补齐，而不是二选一。
- **`linear_local` 是生产扩展路径**：全图 `linear` 在当前 90 段小图上质量相同、
  耗时略低，但会扫描整张图；百万级数据必须使用有界候选、局部 PPR 或离线预计算。
- **纯向量最快但覆盖最低**：适合低延迟和粗召回，不适合单独承担多跳检索。
- **v0 不适合作为当前基线继续优化**：多跳 `All@15` 只有 40%，单跳 Recall@10/15
  也已停止增长，说明其候选和扩展上限较早饱和。
- **当前结论仍有数据边界**：所有结果来自单一内部 PDF、90 个段落和弱标注问题，
  不能替代人工相关性标注、公开中文检索基准和百万级生产压测。
