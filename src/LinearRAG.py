...
for logger_name in ["elasticsearch", "elastic_transport", "neo4j", "urllib3", "spacy", "transformers"]:
...
class LinearRAG:
    ...
    def retrieve(self, question: str, index_names: List[str], top_k: int = 5):
        ...
        embedding_question = f"下面是一个问题，从数据库中检索到问题相关的段落\n问题：{question}"
    ...
    def _build_knn_query(self, query_vector, k, num_candidates=100, filter_type="passage"):
        """构建ES KNN查询的通用方法"""
    ...
    def _fallback_vector_search(self, question_embedding, index_names: List[str], top_k=5):
        """当没有实体时，在多个索引中进行向量检索"""
        ...
        es_results = self._execute_es_search(
            index_names,
            knn_query,
            top_k
        )
    ...
    def _get_seed_entities_from_es(self, question: str, index_names: List[str]) -> List[Dict]:
        """从多个索引中检索种子实体"""
        question_entities = list(self.spacy_ner.question_ner(question))
        if len(question_entities) == 0:
            return []
        
        question_entity_embeddings = self._encode_texts(question_entities, is_batch=True)
        seed_entities = []
                ...
                for passage_result in passage_results:
                    ...
                    try:
                        ...
                        if vec_data:
                            ...
                            passage_similarity = np.dot(passage_vector, question_embedding.T).flatten()[0]
                    except: pass
                    ...
                    entity_results = self._safe_execute(
                        lambda: session.run(cypher_query_entity, passage_id=passage_id, index_names=index_names).data(),
                        default_return=[],
                    ...
                    for entity_result in entity_results:
                        ...
                        if not next_entity_id: continue
                        next_entity_score = entity_score * passage_similarity
                        if next_entity_score < self.config.iteration_threshold: continue
    ...
    def _calculate_passage_scores_neo4j(
        ...
        knn_query = self._build_knn_query(question_embedding, k=50, num_candidates=100, filter_type="passage")
        ...
        for idx, hit in enumerate(hits):
            ...
            passage_cache[passage_id] = {"text": passage_text, "file_id": source.get("file_id", "")}
            ...
            for entity_id, entity_score in entity_scores.items():
                ...
                result = self._safe_execute(
                    lambda: session.run(cypher_query, entity_id=entity_id, passage_id=passage_id, index_names=index_names).data(),
                    default_return=[]
                )
            ...
            passage_scores[passage_id] = 0.6 * dpr_scores[idx] + math.log(1 + total_entity_bonus)
    ...
    def _run_pagerank_and_filter(
        ...
        all_node_ids = list(set(list(entity_scores.keys()) + list(passage_scores.keys())))
        if not all_node_ids:
            return []
        ...
        try:
            ...
            fallback = [{"hash_id": k, "score": v, "pagerank_score": 0, "base_score": v} 
    ...
    def _graph_search_with_neo4j(
        ...
        with self.neo4j_driver.get_session() as session:
            # 1. 计算实体扩散分数
            entity_scores = self._calculate_entity_scores_neo4j(session, seed_entities, question_embedding, index_names)
            
            # 2. 计算段落初步分数
            passage_scores, _ = self._calculate_passage_scores_neo4j(session, question_embedding, entity_scores, index_names)
            
            # 3. PageRank 重新排序
            final_passages = self._run_pagerank_and_filter(session, entity_scores, passage_scores)
    ...
    def index(self, passages: list,kb_name):  
        ...
        new_hash_id_to_passage,all_hash_id_to_passage = self.insert_text(passages,self.embedding_model, self.config.batch_size, "passage",kb_name,self.es_client)
        ...
        # NER (仅新文档)
        new_passage_hash_id_to_entities = self.spacy_ner.batch_ner(new_hash_id_to_passage, self.config.max_workers)
        
        # 获取entity_node信息
        entity_node_info = self.extract_entity_info(new_passage_hash_id_to_entities,new_hash_id_to_passage, passages)
        ...
        # 这里传入不带序号的passage
        graph = self.augment_graph(active_nodes, all_hash_id_to_entity, all_hash_id_to_passage,graph,node_to_node_stats)
    ...
    def save_igraph(self, kb_name, graph, entity_node_info, passages):
        ...
        if entity_node_info and "text" in entity_node_info:
            for text, f_id in zip(entity_node_info["text"], entity_node_info["file_id"]):
        ...
        for i in range(0, total_nodes, node_batch_size):
            ...
            if batch:
                ...
                for attempt in range(3):
                    try:
                        self.neo4j_driver.execute_query(create_nodes_cypher, {"batch_data": batch})
                        break
        ...
        for i in range(0, total_edges, edge_batch_size):
            ...
            for attempt in range(max_retries):
                try:
                    self.neo4j_driver.execute_query(create_edges_cypher, {"batch_data": batch})
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f" [警告] 关系写入发生死锁，正在重试 (第 {attempt+1} 次)... Error: {str(e)[:50]}")
    ...
    def augment_graph(self, restrict_to_nodes,new_hash_id_to_entity, all_hash_id_to_passage,graph,node_to_node_stats):
        # 将过滤集合传递给 add_nodes
        graph = self.add_nodes(restrict_to_nodes,new_hash_id_to_entity, all_hash_id_to_passage,graph)
        graph = self.add_edges(graph,node_to_node_stats)
        return graph

    #将实体和段落作为“顶点（Vertex）”添加到图对象中。
    def add_nodes(self, restrict_to_nodes,new_hash_id_to_entity, all_hash_id_to_passage,graph):
        """
        修改后的 add_nodes:
        ...
        """
        # 检查图中已有的节点
        existing_nodes_in_graph = {v["name"]: v for v in graph.vs if "name" in v.attributes()} 
        ...
        # 5. 重建索引
        self.node_name_to_vertex_idx = {v["name"]: v.index for v in graph.vs if "name" in v.attributes()}   
    ...
    def add_edges(self,graph,node_to_node_stats):
        ...
        for node_hash_id, node_to_node_stats in node_to_node_stats.items():
            for neighbor_hash_id, weight in node_to_node_stats.items():
                ...
                if node_hash_id in self.node_name_to_vertex_idx and neighbor_hash_id in self.node_name_to_vertex_idx:
    ...
    def add_entity_to_passage_edges(self, new_passage_hash_id_to_entities, new_hash_id_to_passage, new_hash_id_to_entity,node_to_node_stats):
    ...
    def insert_text(self, passages_dict, embedding_model, batch_size, type, index_name, es_client):
        es = Customize_Elastic(es_client)
        
        # 1. 基础校验
        if not passages_dict or "text" not in passages_dict or not passages_dict["text"]:
            return {}, {}
    ...
    def extract_entity_info(self,new_passage_hash_id_to_entities, new_hash_id_to_passage, passages):
        """
        ...
        for hash_id, entities in new_passage_hash_id_to_entities.items():
            ...
            if passage_text in text_to_index_map:
                ...
                current_file_name = passages["file_name"][idx] if idx < len(passages["file_name"]) else "Unknown"
                current_file_id = passages["file_id"][idx] if idx < len(passages["file_id"]) else "Unknown"
                ...
                print(f"Warning: Passage text for hash {hash_id} not found in passages source.")
    ...
    def delete_files(self, index_name, file_ids):
            ...
            try:
                es_query = {
                    "query": {
                        "terms": {
                            "file_id.keyword": file_ids
                        }
                    }
                }
                res = self.es_client.delete_by_query(index=index_name, body=es_query, refresh=True)
            ...
            try:
                with self.neo4j_driver.get_session() as session:
                    ...
                    print(f"[Neo4j] 找到 {total_nodes} 个相关节点，开始按 ID 顺序分批处理...")
                    ...
                    print(f"[Neo4j] 操作完成。共处理节点 {total_nodes} 个，实际物理删除节点 {deleted_count} 个。")
